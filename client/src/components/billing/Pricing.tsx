...
const renderPricingButton = ({
  ...
  // Case 1: User has active subscription for the product
  if (
    user &&
    subscription &&
  ...
  // Case 3: Logged in user with no subs or different subscription
  if (user && !subscription) {
    return (
      <Button
        className="mt-8 w-full font-semibold"
...
const Pricing = ({
    ...
    if (!sessionId) {
      return router.push(
        getErrorRedirect(
          ...
          "Please try again later or contact ducbinhnguyen23@gmail.com."
        )
      );
    }
  ...
  return (
    <section
      className={
        (cn("max-w-7xl mx-auto py-16 px-8 w-full flex flex-col"), className)
      }
    >
      {showInterval && (
